...
X = np.load('finalX.npy')  
y = np.load('finaly.npy')   
last_frame_of_x = np.load('finallastframe.npy')  
X1 = np.load('finalX1.npy')  
y1 = np.load('finaly1.npy')  
...
# train_dataset = VideoDataset(X1[:val_idx], flatten_y[:val_idx])
# validation_dataset = VideoDataset(X1[val_idx:], flatten_y[val_idx:])

# test_dataset = VideoDataset(X1[idx:], flatten_y[idx:])

train_dataset = VideoDataset(X1[::DRR], flatten_y[::DRR]) #32000
validation_dataset = VideoDataset(X1[val_idx::DRR], flatten_y[val_idx::DRR])
...
if train_flag:
    ...
    for epoch in range(current_epoch, num_epochs):
        ...
        with torch.no_grad():
            ...
            merged_predicted_val_y = torch.cat(predicted_val_y, dim=0)
            # Convert the merged tensor to a NumPy array
            merged_predicted_val_y_numpy = merged_predicted_val_y.numpy()
            np.save(f'merged_predicted_val_y_{model_name}_numpy.npy', merged_predicted_val_y_numpy)
...
if test_flag:
    ...
    merged_predicted_tes_y = torch.cat(predicted_tes_y, dim=0)
    # Convert the merged tensor to a NumPy array
    merged_predicted_tes_y_numpy = merged_predicted_tes_y.numpy()
    np.save(f'merged_predicted_tes_y_numpy.npy', merged_predicted_tes_y_numpy)
