...
from data_creator import generate_base_frame,generate_moved_frame,visualize_frames
...
frames = [base_frame]
for _ in range(60):
    # Randomly decide horizontal and vertical velocities
    # horizontal_velocity = np.random.randint(-max_velocity, max_velocity + 1)
    horizontal_velocity = 1
    # vertical_velocity = np.random.randint(min_velocity, max_velocity + 1)
    vertical_velocity= 1
...
# if DRR != 0:
#     train_dataset = VideoDataset(x_train_duplicate[::DRR], flatten_y[::DRR]) 
#     # validation_dataset = VideoDataset(x_train_duplicate[val_idx::DRR], flatten_y[val_idx::DRR])
# else:
